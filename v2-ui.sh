...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/li155135236/v2-ui/master/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
install_bbr() {
    bash <(curl -L -s https://raw.githubusercontent.com/li155135236/v2-ui/master/bbr.sh)
    if [[ $? == 0 ]]; then
        echo ""
...
update_shell() {
    wget -O /usr/bin/v2-ui -N --no-check-certificate https://github.com/li155135236/v2-ui/raw/master/v2-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
