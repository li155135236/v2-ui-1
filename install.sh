...
install_v2-ui() {
    ...
    last_version=$(curl -Ls "https://api.github.com/repos/li155135236/v2-ui/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
    echo -e "检测到v2-ui最新版本：${last_version}，开始安装"
    wget -N --no-check-certificate -O /usr/local/v2-ui-linux.tar.gz https://github.com/li155135236/v2-ui/raw/master/v2-ui-linux.tar.gz
    if [[ $? -ne 0 ]]; then
    ...
    echo -e ""
    curl -o /usr/bin/v2-ui -Ls https://raw.githubusercontent.com/li155135236/v2-ui/master/v2-ui.sh
